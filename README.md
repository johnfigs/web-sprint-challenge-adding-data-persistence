...
- A relation database is a series of tables that uses primary keys and foreign keys (that point back to primary keys) to explain table relationships. SQL is the querying language whose syntax allows you to select, join, filter (where), or use aggregate functions to access the data within those tables. 

2. Why do tables need a Primary Key?

- Each table needs a primary key to create a unique identifier for the row.
...
- You need a junction table or cross reference table that allows you to reference the primary keys of two separate tables as foreign keys. 
